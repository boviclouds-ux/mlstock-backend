...
const { protect, requireAdmin, requireDemand } = require('../middleware/authMiddleware');
...
/* ═══════════════════════════════════════════════════════════
   GET /api/quotas/data
   Répond TOUJOURS status 200 + { semences, consommables }
   Chaque ligne inclut le champ `periode` (depuis Campagne.periode).

   Cloisonnement bénéficiaire :
   Un utilisateur non-admin (canDemand) ne reçoit que les dotations
   associées à sa propre coopérative (identifiée par user.entite).
═══════════════════════════════════════════════════════════ */
router.get('/data', protect, requireDemand, async (req, res) => {
  const result = { semences: [], consommables: [] };

  try {
    /* ── Filtre MongoDB : non-admin → ses dotations uniquement ── */
    const dbFilter = {};
    if (!req.user.permissions?.isAdmin) {
      const coop = await Cooperative.findOne({ nom: req.user.entite }).lean();
      if (!coop) return res.status(200).json(result);
...
/* ═══════════════════════════════════════════════════════════
   POST /api/quotas
   Crée une Campagne + les Dotations + les Ordres d'Expédition.

   Corps attendu :
   {
     categorieId : "semences" | "consommables"
     article     : "Holstein – BENNER JESUALDO"
     volumeTotal : 1000
     periode     : "Juin 2026"
     lignes      : [{ cooperative, region, alloue, repartGenetique? }, ...]
   }
═══════════════════════════════════════════════════════════ */
router.post('/', protect, requireAdmin, async (req, res) => {
  ...
  if (!meta) {
    return res.status(400).json({ message: `categorieId invalide : ${categorieId}. Valeurs acceptées : semences, consommables.` });
  }
  ...
  try {
    ...
    for (const ligne of lignes) {
      ...
      const dotation = await Dotation.findOneAndUpdate(
        ...
        { $set: { alloue: Number(ligne.alloue) || 0, consomme: 0, statut: 'normal', locked: false } },
        { new: true, upsert: true, setDefaultsOnInsert: true }
      );
      dotationsCreees.push(dotation._id);
    }

    /* 4. Créer les Ordres d'Expédition (ORDRE_ADMIN) pour les semences ─ */
    const ordresCreees = [];
          ...
          const tx = await Transaction.create({
            ...
            repartGenetique: Array.isArray(ligne.repartGenetique)
              ? ligne.repartGenetique.map(g => ({
                  ...
                  nni:              g.nni              ?? '',
                  couleur:          g.couleur          ?? '',
                  conteneurSemence: g.conteneurSemence ?? '',
...
/* ═══════════════════════════════════════════════════════════
   ...
   Supprime toutes les Dotations (tableau vierge).
═══════════════════════════════════════════════════════════ */
router.patch('/reinitialiser', protect, requireAdmin, async (req, res) => {
  try {
    const result = await Dotation.deleteMany({});
    return res.status(200).json({
      message:      `Réinitialisation effectuée : ${result.deletedCount} dotation(s) supprimée(s).`,
...
/* ═══════════════════════════════════════════════════════════
   ...
   Clôture toutes les campagnes actives et verrouille leurs dotations.
═══════════════════════════════════════════════════════════ */
router.patch('/cloturer', protect, requireAdmin, async (req, res) => {
  try {
