...
export default function ProfilUtilisateur({ dark, onToggleDark }) {
  ...
  const lbl   = "block text-xs font-bold text-slate-300 dark:text-slate-400 uppercase tracking-wide mb-1.5";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-300 dark:text-slate-400 mt-0.5">Gérez vos informations, préférences et sécurité.</p>
        </div>
        {saved && (
        ...
        <div className="flex items-center gap-5 mb-6 pb-6 border-b border-slate-100 dark:border-slate-700">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-300">Membre depuis {UTILISATEUR.dateCreation}</span>
            </div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-300 mt-0.5">Basculer entre le mode clair et sombre</p>
            </div>
            <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 p-1 rounded-xl shrink-0">
              <button onClick={()=>onToggleDark(false)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${!dark?"bg-white dark:bg-slate-600 text-slate-800 dark:text-white shadow-sm":"text-slate-300 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300"}`}>
                <Sun size={13}/> Clair
              </button>
              <button onClick={()=>onToggleDark(true)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${dark?"bg-slate-800 text-white shadow-sm":"text-slate-300 hover:text-slate-700"}`}>
                <Moon size={13}/> Sombre
              </button>
            </div>
          </div>
          ...
          */}
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-300 mt-0.5">Actuellement : <strong className="text-slate-400 dark:text-slate-300">{langue}</strong></p>
            </div>
            <div className="relative shrink-0">
          ...
          */}
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-300 mt-0.5">Alertes de quotas, réceptions et dérogations</p>
            </div>
      ...
      <div className={card}>
        ...
        <div className="space-y-4">
          ...
          {[
            ...
            <div key={label}>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={()=>setShow(v=>!v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-400 dark:hover:text-slate-300 transition-colors">
          ...
          <div className="rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30 px-4 py-3">
            <p className="text-[10px] font-bold text-slate-300 dark:text-slate-400 uppercase tracking-wide mb-2">Règles du mot de passe</p>
            <div className="space-y-1">
              {[
                ...
                <div key={rule} className="flex items-center gap-2 text-[11px]">
                  ...
                  <span className={met?"text-emerald-600 dark:text-emerald-400 font-medium":"text-slate-400 dark:text-slate-300"}>{rule}</span>
                </div>
              ))}
            </div>
          </div>
